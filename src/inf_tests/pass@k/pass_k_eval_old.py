...
def run_pass_k_evaluation(k: int = pass_k_num, temperature: float = 0.8) -> dict:
...
def main():
    ...
    try:
        ...
        # Save detailed results (append to existing file or create new)
        current_result = {
            "metrics": metrics,
            "solutions": solutions
        }
        ...
        if os.path.exists(args.output):
            try:
                ...
                # If existing data is a single result, convert to list
                if isinstance(existing_data, dict) and "metrics" in existing_data:
                    existing_data = [existing_data]
                # If existing data is already a list, append to it
        ...
        print(f"\nResults appended to: {args.output} (total runs: {len(existing_data)})")
